...
class DiabetesLTNRuntime:
    ...
    def __init__(self, checkpoint_dir: Path):
        ...
        self.rule_bank.load_state_dict(rule_bank_state, strict=True)
    ...
    def explain(self, h: str, r: str, t: str, top_k: int = 3):
        ...
        μ, groundings = evaluate_rules(self.model, triples, subset, self.entity_ids, return_groundings=True)  # (|subset|,1)

        scored = sorted(
            [(rule.lam.item(), μ[i, 0].item(), rule, groundings[i]) for i, rule in enumerate(subset)],
            key=lambda x: x[0] * x[1],
            reverse=True,
        )[:top_k]

        return [
            {"rule": rule, "lambda": lam, "mu": mu, "support": lam * mu, "groundings": groundings}
            for lam, mu, rule, groundings in scored
        ]
